...
2. Tiles rendered in Pixi are generator-driven via `getTileAt(seed, x, y)`/`generateChunk(seed, cx, cy)` with a fixed default seed.
...
12. Renderer keeps a `Map<string, Chunk>` of loaded chunks and chunk keys use `${cx}:${cy}` in render state.
13. A debug overlay shows: seed, camera world coordinates, zoom, and loaded chunk count.
14. Changing seed from the overlay regenerates rendered chunks.
15. Running `npm test` passes.
16. Running `npm run build` succeeds without errors.
