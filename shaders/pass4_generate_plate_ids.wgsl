...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let macro_freq = max(params.plate_warp_frequency * 0.15, 0.1);
  let micro_freq = max(params.plate_warp_frequency * 3.0, 1.0);
  let safe_macro_amp = min(params.plate_warp_amplitude, 0.25 / macro_freq);
  let safe_micro_amp = min(params.plate_warp_amplitude * 0.15, 0.05 / micro_freq);
